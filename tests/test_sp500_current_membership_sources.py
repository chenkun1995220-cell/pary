...
class Sp500CurrentMembershipSourcesTests(unittest.TestCase):
    ...
    def test_builds_secondary_rows_from_public_constituents_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(payload["current_screening_allowed"])
            self.assertEqual(payload["source_usage_scope"], "current_screening_only")
            self.assertFalse(payload["verified_historical_evidence_allowed"])
            self.assertFalse(payload["formal_backtest_upgrade_allowed"])
    ...
    def test_cli_builds_crosscheck_substitute_rows_from_xlsx(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["source_usage_scope"], "current_screening_only")
            report_text = report.read_text(encoding="utf-8-sig")
            self.assertIn("source_usage_scope: current_screening_only", report_text)
            self.assertIn("verified_historical_evidence_allowed: false", report_text)
            self.assertFalse(payload["formal_backtest_upgrade_allowed"])
