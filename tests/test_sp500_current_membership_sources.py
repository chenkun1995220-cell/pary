...
class Sp500CurrentMembershipSourcesTests(unittest.TestCase):
    ...
    def test_cli_can_write_empty_report_when_fetch_source_is_unavailable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("source_file_inbox_size_bytes", request_text)
            self.assertIn("source_file_inbox_sha256", request_text)
            self.assertIn("source_file_inbox_modified_at", request_text)
