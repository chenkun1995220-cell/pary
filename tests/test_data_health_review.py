...
class DataHealthReviewTests(unittest.TestCase):
    def test_builds_review_with_refetch_gap_candidate_impact_and_boundaries(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["data_health_triage_status"], "monitor_only")
            self.assertEqual(payload["data_health_triage_decision"], "monitor_next_run")
            self.assertEqual(payload["data_health_triage_counts"]["candidate_blocking"], 0)
            self.assertEqual(payload["data_health_triage_counts"]["refetch_required"], 0)
            self.assertEqual(payload["data_health_triage_counts"]["monitor_only"], 3)
            self.assertFalse(payload["candidate_delivery_blocked"])
            ...
            self.assertIn("## 数据健康三层分流", report)
            self.assertIn("monitor_only", report)
            self.assertIn("candidate_blocking", report)
            self.assertIn("refetch_required", report)
