...
class CandidateFindingsReviewTests(unittest.TestCase):
    def test_builds_review_from_valuation_targets_and_risk_table(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["risk_reduction_status"], "action_required")
            self.assertEqual(payload["risk_reduction_decision"], "manual_review_queue_ready")
            self.assertEqual(payload["risk_reduction_priority_market"], "测试市场")
            self.assertEqual(payload["risk_reduction_target_action_count_after_close"], 0)
            self.assertIn("关闭全部风险行动项", payload["risk_reduction_close_condition"])
            self.assertEqual(payload["risk_reduction_queue"][0]["market"], "测试市场")
            self.assertEqual(payload["risk_reduction_queue"][0]["ticker"], "BBB")
            ...
            self.assertIn("## 风险压降清单", report)
            self.assertIn("manual_review_queue_ready", report)
            self.assertIn("| defer_research | 1 |", report)
