...
INPUT_SPECS = {
    ...
    "membership_evidence_apply_confirmation_status": {
        "path": "outputs/automation/latest_membership_evidence_apply_confirmation_status.json",
        "schema_field": "confirmation_schema",
        "schema_value": "membership_evidence_apply_confirmation_status",
    ...
    "membership_evidence_supplement_queue": {
        ...
        "schema_value": "membership_evidence_supplement_queue",
    ...
    "one_week_forecast_shadow_parameter_plan": {
        "path": "outputs/automation/latest_one_week_forecast_shadow_parameter_plan.json",
        "schema_field": "plan_schema",
        "schema_value": "one_week_forecast_shadow_parameter_plan",
        "version_field": "plan_version",
        "version_value": 1,
    },
...
def _sp500_current_membership_source_inbox_status_reasons(payload, project_root=None):
    ...
    if _sp500_source_inbox_fingerprint_mismatch(payload, project_root):
...
def _sp500_source_inbox_fingerprint_mismatch(payload, project_root=None):
    if payload.get("source_file_inbox_exists") is not True:
        return False
    inbox_file = str(payload.get("source_file_inbox", "") or "").strip()
    if not inbox_file:
        return False
    ...
    actual_modified_at = datetime.fromtimestamp(stat.st_mtime, tz=timezone.utc).isoformat()
    return (
        _int_value(payload.get("source_file_inbox_size_bytes"), 0) != stat.st_size
        or str(payload.get("source_file_inbox_sha256", "") or "").strip()
        != hashlib.sha256(inbox_path.read_bytes()).hexdigest()
...
def _source_file_request_manifest_fields_missing(path):
    try:
        text = Path(path).read_text(encoding="utf-8-sig")
    except OSError:
        return True
...
def _sp500_current_membership_source_action_item_link_reasons(source_status, action_items):
    ...
    if expected_action == "provide_official_constituents_csv":
        ...
        if _official_csv_action_item_source_file_paths_missing(source):
            return [
...
def _official_csv_action_item_source_inbox_status_details_missing(source):
    text = str(source or "")
    required_markers = [
        "source_file_inbox_status:",
        "source_file_inbox_next_action:",
        "source_file_inbox_parsed_official_ticker_count:",
        "source_file_inbox_intake_missing_count:",
    ]
    return any(marker not in text for marker in required_markers)


def _official_csv_action_item_source_file_paths_missing(source):
    text = str(source or "")
    required_markers = [
        "source_file_request_file:",
        "source_file_inbox:",
        "source_file_inbox_exists:",
